...
* **Group size**: ``ARRAY_AGG()`` assembles the array in memory as the rows of a group are read, and a query paused part-way through a group returns the elements collected so far in its continuation. Neither the array size in memory nor the continuation size is limited, and since they both grow with the group, a very large group can use a substantial amount of memory and produce a large continuation. To limit this cost, minimize what you collect into the array, for example by collecting only primary keys rather than entire records. Where the aggregation is grouped, also consider choosing fine-grained grouping keys to keep groups to a moderate size (and avoid an ungrouped ``ARRAY_AGG()``, which collects the entire input as a single group). If a group can grow without a natural bound, you may want to avoid applying ``ARRAY_AGG()`` in SQL and instead assemble the array in the client application as rows arrive. A flat ``SELECT`` ordered by the grouping key (with an available index to serve that order) streams the same data with a continuation that records only a position in the scan, at the cost of repeating the grouping key on every row, and of tracking group boundaries across pages in the client. In exchange the continuation stays small, and the client can process rows as they arrive rather than materializing the whole array.
