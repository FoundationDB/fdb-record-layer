    ...
    public Object visitPreparedStatementParameter(@Nonnull RelationalParser.PreparedStatementParameterContext ctx) {
        ...
        if (ctx.QUESTION() != null) {
            ...
            if (!preparedStatementParameters.hasNamedParamValue(parameterName)
                ...
                // without binding it (see MutablePlanGenerationContext.processNamedPreparedParam, which also resolves
                // the declaration into a type). The parameter hash necessarily omits the value there is none of; that is safe
