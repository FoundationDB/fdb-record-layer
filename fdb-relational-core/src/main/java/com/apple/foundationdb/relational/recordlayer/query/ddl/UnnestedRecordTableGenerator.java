...
/**
 * Generates the unnested synthetic table an index has to be defined on. A joined synthetic table, once there is one,
 * would be generated by its own counterpart rather than here.
 * <p>
 * An index over an unnesting is usually maintained from the stored table, with a fan-out key expression. That works
...
final class UnnestedRecordTableGenerator {
    ...
    /**
     * A generator for the index's synthetic table, if it needs one at all, by the adjacency rule described on this
     * class.
     * <p>
    ...
    static Optional<UnnestedRecordTableGenerator> initIfNeeded(@Nonnull final RecordLayerSchemaTemplate.Builder schemaTemplateBuilder,
        ...
        // A synthetic table exists for one index, so its name carries that index's, which is what keeps two indexes
        // over the same array from sharing one.
