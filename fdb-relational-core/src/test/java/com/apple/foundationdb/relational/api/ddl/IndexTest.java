...
public class IndexTest {
     ...
     */
    /**
     * An aggregate whose grouping columns force a synthetic table: {@code M.x} and {@code SUM(M.x2)} are read through the
     * same unnesting with {@code t.p} between them. Aggregating over synthetic records rather than stored ones is not
     * worked out, so this is rejected rather than falling back to a fan-out that cannot express it either.
     */
    @Test
    void createAggregateIndexOverUnnestedSyntheticTableIsNotSupported() throws Exception {
        ...
        shouldFailWith(stmt, ErrorCode.UNSUPPORTED_OPERATION,
                "cannot be defined on an unnested synthetic table");
    }
    ...
    void createVersionIndexWithRepeatedNestedSplitByVersion() throws Exception {
        final String stmt = "CREATE SCHEMA TEMPLATE test_template " +
                ...
                "WITH OPTIONS(store_row_versions=true)";
        // As above, but the column separating X.col2 from X.col3/X.col4 is the row version. The split forces a synthetic
        // table, and a synthetic record has no version of its own, so this shape is rejected rather than reading the
        // version through whichever constituent happened to carry it.
        shouldFailWith(stmt, ErrorCode.UNSUPPORTED_OPERATION,
                "a version column cannot be part of an index over an unnested synthetic table");
    }
