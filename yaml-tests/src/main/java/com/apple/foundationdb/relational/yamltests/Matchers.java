...
import com.apple.foundationdb.relational.yamltests.tags.IgnoreColumnNameTag;
...
public class Matchers {
    ...
    private static ResultSetMatchResult matchMap(@Nonnull final Map<?, ?> expected,
        ...
        for (final var entry : expected.entrySet()) {
            ...
            final var actualField = entry.getValue() == null || entry.getKey() instanceof IgnoreColumnNameTag.IgnoreColumnNameMatcher ?
                                    entryByNumberAccessor.apply(counter) : entryByNameAccessor.apply(string(entry.getKey()));
            final var currentCellRef = entry.getValue() == null ? "pos<" + counter + ">" : entry.getKey().toString();
