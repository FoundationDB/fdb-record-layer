/*
 * LuceneOptimizedBlockTreeTermsReader.java
 *
...
public final class LuceneOptimizedBlockTreeTermsReader extends FieldsProducer {
    ...
    public final IndexInput indexIn;

    //private static final boolean DEBUG = BlockTreeTermsWriter.DEBUG;

    // Reads the terms dict entries, to gather state to
    // produce DocsEnum on demand
    public final PostingsReaderBase postingsReader;

    private final Map<String, LuceneOptimizedFieldReader> fieldMap;
    ...
    /** Sole constructor. */
    public LuceneOptimizedBlockTreeTermsReader(PostingsReaderBase postingsReader, SegmentReadState state) throws IOException {
        ...
        try {
            // The open input is moved around to allow both pieces to be fetched asynchronously under the covers.
            ...
            Map<String, LuceneOptimizedFieldReader> fieldMap = null;
            ...
            try (ChecksumIndexInput metaIn = version >= VERSION_META_FILE ? state.directory.openChecksumInput(metaName, state.context) : null) {
                try {
                    ...
                    for (int i = 0; i < numFields; ++i) {
                        ...
                        LuceneOptimizedFieldReader previous = fieldMap.put(fieldInfo.name,
                                new LuceneOptimizedFieldReader(this, fieldInfo, numTerms, rootCode, sumTotalTermFreq, sumDocFreq, docCount,
                    ...
                    //if (metaIn != null) {
                        // CodecUtil.checkFooter(metaIn, priorE); // Do not search to end, optimization
                    //} else
                    if (priorE != null) {
            ...
            if (version >= VERSION_META_FILE) {
                ...
                // The footer operation in the checksum will force another block to be loaded...
    ...
    public long ramBytesUsed() {
        ...
        for(LuceneOptimizedFieldReader reader : fieldMap.values()) {
