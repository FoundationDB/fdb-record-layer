/*
 * LuceneOptimizedIntersectTermsEnum.java
 *
...
public final class LuceneOptimizedIntersectTermsEnum extends BaseTermsEnum {
    ...
    LuceneOptimizedIntersectTermsEnumFrame[] stack;
    ...
    private LuceneOptimizedIntersectTermsEnumFrame currentFrame;
    ...
    public LuceneOptimizedIntersectTermsEnum(LuceneOptimizedFieldReader fr, Automaton automaton, RunAutomaton runAutomaton, BytesRef commonSuffix, BytesRef startTerm) throws IOException {
        this.fr = fr;
        ...
        stack = new LuceneOptimizedIntersectTermsEnumFrame[5];
        for(int idx=0;idx<stack.length;idx++) {
            stack[idx] = new LuceneOptimizedIntersectTermsEnumFrame(this, idx);
        }
        ...
        final LuceneOptimizedIntersectTermsEnumFrame f = stack[0];
    ...
    private LuceneOptimizedIntersectTermsEnumFrame getFrame(int ord) throws IOException {
        if (ord >= stack.length) {
            final LuceneOptimizedIntersectTermsEnumFrame[] next = new LuceneOptimizedIntersectTermsEnumFrame[ArrayUtil.oversize(1 + ord, RamUsageEstimator.NUM_BYTES_OBJECT_REF)];
            System.arraycopy(stack, 0, next, 0, stack.length);
            for(int stackOrd=stack.length;stackOrd<next.length;stackOrd++) {
                next[stackOrd] = new LuceneOptimizedIntersectTermsEnumFrame(this, stackOrd);
            }
            stack = next;
        }
    ...
    private LuceneOptimizedIntersectTermsEnumFrame pushFrame(int state) throws IOException {
        assert currentFrame != null;

        //final LuceneOptimizedIntersectTermsEnumFrame f = getFrame(currentFrame == null ? 0 : 1+currentFrame.ord);
        final LuceneOptimizedIntersectTermsEnumFrame f = getFrame(1+currentFrame.ord);
