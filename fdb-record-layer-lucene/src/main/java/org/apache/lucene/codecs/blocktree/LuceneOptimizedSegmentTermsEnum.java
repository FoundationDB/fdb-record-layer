/*
 * LuceneOptimizedSegmentTermsEnum.java
 *
...
import com.apple.foundationdb.record.lucene.codec.LuceneOptimizedFieldReader;
...
public final class LuceneOptimizedSegmentTermsEnum extends BaseTermsEnum {

    // Lazy init:
    IndexInput in;

    private LuceneOptimizedSegmentTermsEnumFrame[] stack;
    private final LuceneOptimizedSegmentTermsEnumFrame staticFrame;
    LuceneOptimizedSegmentTermsEnumFrame currentFrame;
    boolean termExists;
    final LuceneOptimizedFieldReader fr;
    ...
    public LuceneOptimizedSegmentTermsEnum(LuceneOptimizedFieldReader fr) throws IOException {
        this.fr = fr;

        // if (DEBUG) {
        //   System.out.println("BTTR.init seg=" + fr.parent.segment);
        // }
        stack = new LuceneOptimizedSegmentTermsEnumFrame[0];

        // Used to hold seek by TermState, or cached seek
        staticFrame = new LuceneOptimizedSegmentTermsEnumFrame(this, -1);
    ...
    public LuceneOptimizedStats computeBlockStats() throws IOException {

        LuceneOptimizedStats stats = new LuceneOptimizedStats(fr.parent.segment, fr.fieldInfo.name);
        if (fr.index != null) {
    ...
    private LuceneOptimizedSegmentTermsEnumFrame getFrame(int ord) throws IOException {
        if (ord >= stack.length) {
            final LuceneOptimizedSegmentTermsEnumFrame[] next = new LuceneOptimizedSegmentTermsEnumFrame[ArrayUtil.oversize(1 + ord, RamUsageEstimator.NUM_BYTES_OBJECT_REF)];
            System.arraycopy(stack, 0, next, 0, stack.length);
            for(int stackOrd=stack.length;stackOrd<next.length;stackOrd++) {
                next[stackOrd] = new LuceneOptimizedSegmentTermsEnumFrame(this, stackOrd);
            }
            stack = next;
        }
    ...
    LuceneOptimizedSegmentTermsEnumFrame pushFrame(FST.Arc<BytesRef> arc, BytesRef frameData, int length) throws IOException {
        scratchReader.reset(frameData.bytes, frameData.offset, frameData.length);
        final long code = scratchReader.readVLong();
        final long fpSeek = code >>> BlockTreeTermsReader.OUTPUT_FLAGS_NUM_BITS;
        final LuceneOptimizedSegmentTermsEnumFrame f = getFrame(1+currentFrame.ord);
    ...
    LuceneOptimizedSegmentTermsEnumFrame pushFrame(FST.Arc<BytesRef> arc, long fp, int length) throws IOException {
        final LuceneOptimizedSegmentTermsEnumFrame f = getFrame(1+currentFrame.ord);
        f.arc = arc;
  ...
  */
    ...
    public boolean seekExact(BytesRef target) throws IOException {
        ...
        if (currentFrame != staticFrame) {
            ...
            LuceneOptimizedSegmentTermsEnumFrame lastFrame = stack[0];
    ...
    private void printSeekState(PrintStream out) throws IOException {
        if (currentFrame == staticFrame) {
            ...
            while(true) {
                LuceneOptimizedSegmentTermsEnumFrame f = getFrame(ord);
                assert f != null;
