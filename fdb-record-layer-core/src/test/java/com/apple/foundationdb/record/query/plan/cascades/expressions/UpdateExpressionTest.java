...
/**
 * Tests for {@link UpdateExpression}.
 */
class UpdateExpressionTest {
    ...
    void toStringTranslatesEscapedProtoIdentifierBackToUserIdentifier() {
        ...
        final var innerQuantifier = scanQuantifierOver(targetType, "R");
        final var fieldValue = FieldValue.ofFieldName(innerQuantifier.getFlowedObjectValue(), "a");
        final var transformMap = ImmutableMap.<FieldValue.FieldPath, Value>of(fieldValue.getFieldPath(), new LiteralValue<>(1L));
        final var escapedTargetRecordType = ProtoUtils.toProtoBufCompliantName("My.Record");
        final var updateExpression = new UpdateExpression(innerQuantifier, escapedTargetRecordType, targetType, transformMap);

        assertEquals("Update(My.Record, [" + fieldValue.getFieldPath() + "] ", updateExpression.toString());
    }
}
