...
public class FDBModificationQueryTest extends FDBRecordStoreQueryTestBase {
        ...
        try (FDBRecordContext context = openContext()) {
            ...
            plan = cascadesPlanner.planGraph(
                    () -> {
                        ...
                        qun = Quantifier.forEach(Reference.initial(graphExpansionBuilder.build().buildSelectWithResultValue(QuantifiedObjectValue.of(qun))));

                        // make accessors and resolve them
                        qun = Quantifier.forEach(Reference.initial(new DeleteExpression(qun, "RestaurantRecord")));
    ...
    private static Reference insertGraph() {
        ...
        qun = Quantifier.forEach(Reference.initial(new InsertExpression(qun,
                "RestaurantRecord",
    ...
    private static Reference selectRecordsGraph(Function<Quantifier.ForEach, QueryPredicate> predicateCreator) {
        ...
        qun = Quantifier.forEach(Reference.initial(selectExpression));
        return Reference.initial(LogicalSortExpression.unsorted(qun));
    }
        ...
        try (FDBRecordContext context = openContext()) {
            ...
            plan = planGraph(
                    () -> {
                        ...
                        qun = Quantifier.forEach(Reference.initial(new UpdateExpression(qun,
                                "RestaurantRecord",
                                restaurantType,
                    ...
                    () -> {
                        ...
                        final var bestRecord = RecordConstructorValue.ofUnnamed(
                                ImmutableList.of(LiteralValue.ofScalar(400L),
                                        ...
                                        emptyArray(customerType)));
                        final var explodeExpression = new ExplodeExpression(LightArrayConstructorValue.of(bananaRecord, bestRecord));
                        var outerQun = Quantifier.forEach(Reference.initial(explodeExpression));
                        ...
                        final var innerQun = Quantifier.existential(Reference.initial(new UpdateExpression(qun,
                                "RestaurantRecord",
                                restaurantType,
                        ...
                        qun = Quantifier.forEach(Reference.initial(graphExpansionBuilder.build().buildSelectWithResultValue(outerQun.getFlowedObjectValue())));
