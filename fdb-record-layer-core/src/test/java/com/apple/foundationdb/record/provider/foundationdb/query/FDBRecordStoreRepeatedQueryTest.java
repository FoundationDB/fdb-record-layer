...
class FDBRecordStoreRepeatedQueryTest extends FDBRecordStoreQueryTestBase {
    ...
    private Quantifier repeatExpansion(@Nonnull Quantifier baseQun, @Nonnull Function<Value, List<QueryPredicate>> repeatValuePredicates) {
        ...
        Quantifier repeatQun = Quantifier.forEach(Reference.initial(explodeExpression));
        ...
        return Quantifier.forEach(Reference.initial(repeatSelectBuilder.build().buildSelect()));
    }
    ...
    void querySumByRepeatedPredicateOnGroup() {
        ...
        try (FDBRecordContext context = openContext()) {
            ...
            final RecordQueryPlan plan = planGraph(() -> {
                ...
                final Quantifier selectHaving = Quantifier.forEach(Reference.initial(selectHavingBuilder.build().buildSelect()));
                return Reference.initial(LogicalSortExpression.unsorted(selectHaving));
            });
