 ...
 */
public class ValueMaxMatchUsingBoundEquivalenceTest {

    @Nonnull
    BiMap<Equivalence.Wrapper<Value>, Value> translateValueUsingBoundEquivalence(@Nonnull final Value source,
        ...
        translatedSource.preOrderPruningIterator(needle -> {
            ...
            if (found.isEmpty()) {
                ...
                if (toBeMappedTarget == null) {
                    throw new RecordCoreException(String.format("could not pull up %s", haystack));
                }
    ...
    public void testValueTranslation() throws Exception {
        ...
        final var result = translateValueUsingBoundEquivalence(pv, CorrelationIdentifier.of("p"), p_v, CorrelationIdentifier.of("p_"), Map.of(boundEquivalence.wrap(t), t_));
        ...
        final var expectedMapping = Map.of(
                ...
                be.wrap(fv(p, 1)), /* -> */ fv(p_, 0, 1),
                be.wrap(fv(p, 2, 0)), /* -> */ fv(p_, 1, 0),
                be.wrap(fv(p, 3)), /* -> */ fv(p_, 2));
    ...
    public void testValueTranslation4() throws Exception {
        ...
        final var result = translateValueUsingBoundEquivalence(pv, CorrelationIdentifier.of("p"), p_v, CorrelationIdentifier.of("p_"), Map.of(boundEquivalence.wrap(t), t_, boundEquivalence.wrap(s), s_));
    ...
    public void testValueTranslation5() throws Exception {
        ...
        var result = translateValueUsingBoundEquivalence(pv, CorrelationIdentifier.of("p"), p_v, CorrelationIdentifier.of("p_"), Map.of(boundEquivalence.wrap(t), t_));

        final var p = qov("p", pv.getResultType());
        final var p_ = qov("p_", p_v.getResultType());

        var expectedBoundAliasMap = boundIdentitiesMap.combine(AliasMap.of(CorrelationIdentifier.of("p"), CorrelationIdentifier.of("p")));
        Correlated.BoundEquivalence<Value> be = new Correlated.BoundEquivalence<>(expectedBoundAliasMap);
        Map<Equivalence.Wrapper<Value>, Value> expectedMapping = Map.of(
                be.wrap(fv(p, 0)), /* -> */ fv(p_, 0, 0),
                be.wrap(fv(p, 1)), /* -> */ fv(p_, 0, 1),
                be.wrap(fv(p, 2, 0)), /* -> */ fv(p_, 1, 0),
                be.wrap(fv(p, 3)), /* -> */ fv(p_, 2));
        ...
        result = translateValueUsingBoundEquivalence(rv, CorrelationIdentifier.of("r"), r_v, CorrelationIdentifier.of("r_"), expectedMapping);

        final var r = qov("r", pv.getResultType());
        final var r_ = qov("r_", p_v.getResultType());

        expectedBoundAliasMap = boundIdentitiesMap.combine(AliasMap.of(CorrelationIdentifier.of("r"), CorrelationIdentifier.of("r")));
        be = new Correlated.BoundEquivalence<>(expectedBoundAliasMap);
        expectedMapping = Map.of(
                be.wrap(fv(r, 0)), /* -> */ fv(r_, 0, 0)
                );
        Assertions.assertEquals(expectedMapping, result);
    }
    ...
    private static Type.Record r(String... fields) {
        return Type.Record
                .fromFields(Arrays.stream(fields)
                        .map(ValueMaxMatchUsingBoundEquivalenceTest::f)
