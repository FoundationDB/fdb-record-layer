...
/**
 * Matches any (non-strict) descendant with the given matcher. Note that a plan is its own non-strict descendants.
 */
public class DescendantMatcher extends TypeSafeMatcher<RecordQueryPlan> {
    ...
    public boolean matchesSafely(@Nonnull RecordQueryPlan plan) {
        return matcher.matches(plan) || plan.getChildren().stream().anyMatch(this::matchesSafely);
    }
