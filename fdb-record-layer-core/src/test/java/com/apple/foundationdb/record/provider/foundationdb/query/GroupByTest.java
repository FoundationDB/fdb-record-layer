...
public class GroupByTest extends FDBRecordStoreQueryTestBase {
    ...
    private Reference constructBitmapGroupByPlan(int bucketSize, boolean zeroGroup) {
        ...
        // 2. build the group by expression, for that we need the aggregation expression and the grouping expression.
        {
            ...
            final var bitBucketNumValue2FieldValue = (Value)new ArithmeticValue.BitmapBucketOffsetFn().encapsulate(CallSiteArguments.ofPositional(num2, bucketSizeValue));

            final Value bitMapValue = (Value) new NumericAggregationValue.BitmapConstructAggFn().encapsulate(CallSiteArguments.ofPositional((Value) new ArithmeticValue.BitmapBitPositionFn().encapsulate(CallSiteArguments.ofPositional(num2, bucketSizeValue))));
