...
public class AsyncLockCursorTest {
    ...
    @Test
    public void asyncLockCursorTest() throws InterruptedException {
        try (final var context = fdb.openContext()) {
            final var writeLockAndWait1 = getWriteLock(context);
            final var readLockCursor = new AsyncLockCursor<>(identifier, context, new ListCursor<>(ImmutableList.of(1, 2, 3, 4, 5), null), true);
            final var futures = IntStream.rangeClosed(1, 5).mapToObj(ignore -> readLockCursor.onNext()).collect(Collectors.toList());
            final var writeLockAndWait2 = getWriteLock(context);
            ...
            // check that all readLockCursor result's wait
            AsyncLockRegistryTest.checkWaiting(futures);
            AsyncLockRegistryTest.checkWaiting(ImmutableList.of(writeLockAndWait2.getRight()));
            // complete the first write and check that all the readLockCursor result's finish
            writeLockAndWait1.getLeft().complete(null);
            AsyncLockRegistryTest.checkAllCompletedNormally(futures);
            AsyncLockRegistryTest.checkWaiting(ImmutableList.of(writeLockAndWait2.getRight()));
            // close the cursor and check that the other write don't wait
            readLockCursor.close();
            AsyncLockRegistryTest.checkAllCompletedNormally(ImmutableList.of(writeLockAndWait2.getRight()));
        }
    }

    private NonnullPair<CompletableFuture<Void>, CompletableFuture<Void>> getWriteLock(@Nonnull FDBRecordContext context) {
        final var newWriteLock = new CompletableFuture<Void>();
        return NonnullPair.of(newWriteLock, context.getWriteLock(identifier, newWriteLock));
    }
}
