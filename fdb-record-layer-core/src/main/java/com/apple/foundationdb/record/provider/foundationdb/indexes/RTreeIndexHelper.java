...
public class RTreeIndexHelper {
    private static final byte NODE_SLOT_INDEX_SUBSPACE_INDICATOR = 0x00;

    private RTreeIndexHelper() {
    }

    /**
     * Builds the stored {@link RTree.Point} from the coordinate columns of an index entry key.
     */
    @FunctionalInterface
    public interface PointBuilder {
        @Nonnull
        RTree.Point build(@Nonnull List<Object> coordinateItems);
    }

    /**
     * Renders the coordinate portion of a returned index entry key from the coordinates stored in the R-tree.
     */
    @FunctionalInterface
    public interface CoordinateReconstructor {
        @Nonnull
        List<Object> reconstruct(@Nonnull Tuple storedCoordinates);
    }

    /**
     * The subspace holding the R-tree's node-slot secondary index, derived from the index's secondary subspace.
     * @param state the maintainer state
     * @return the node-slot index subspace
     */
    @Nonnull
    public static Subspace nodeSlotIndexSubspace(@Nonnull final IndexMaintainerState state) {
        return state.store.indexSecondarySubspace(state.index).subspace(Tuple.from(NODE_SLOT_INDEX_SUBSPACE_INDICATOR));
    }

    /**
     * Scan the R-tree(s) for the given spatial predicate. Iterates each distinct prefix tuple (each a separate R-tree)
     * within {@code prefixRange}, joining with an R-tree search bounded by {@code mbrPredicate} and {@code suffixRange}
     * and filtered by {@code containsPredicate}.
     *
     * @param state the maintainer state
     * @param config the R-tree configuration
     * @param prefixSize number of leading (prefix/grouping) columns partitioning the index into separate R-trees
     * @param prefixRange range constraining the prefix columns
     * @param suffixRange range constraining the key suffix, applied both as an R-tree bound and a post-filter
     * @param mbrPredicate accepts a node's minimum bounding rectangle if the search should descend into it
     * @param containsPredicate accepts a stored point if it satisfies the (exact) spatial condition
     * @param coordinateReconstructor renders stored coordinates back into the returned index entry key
     * @param continuation continuation from a previous scan, or {@code null}
     * @param scanProperties skip, limit and other scan properties
     * @return a cursor over the matching index entries
     */
    @SuppressWarnings("resource")
    @Nonnull
    public static RecordCursor<IndexEntry> scan(@Nonnull final IndexMaintainerState state,
                                                @Nonnull final RTree.Config config,
                                                final int prefixSize,
                                                @Nonnull final TupleRange prefixRange,
                                                @Nonnull final TupleRange suffixRange,
                                                @Nonnull final Predicate<RTree.Rectangle> mbrPredicate,
                                                @Nonnull final Predicate<RTree.Point> containsPredicate,
                                                @Nonnull final CoordinateReconstructor coordinateReconstructor,
        ...
        final Subspace nodeSlotIndexSubspace = nodeSlotIndexSubspace(state);
        final Executor executor = state.context.getExecutor();
        ...
        return RecordCursor.flatMapPipelined(prefixSkipScan(state, prefixSize, timer, prefixRange, innerScanProperties),
                        (prefixTuple, innerContinuation) -> {
                            ...
                            final RTree rTree = new RTree(rtSubspace, rtNodeSlotIndexSubspace, executor, config,
                            ...
                            return new LazyCursor<>(state.context.acquireReadLock(new LockIdentifier(rtSubspace))
                                    .thenApply(lock -> new AsyncLockCursor<>(lock, new ItemSlotCursor(executor,
                                            rTree.scan(transaction, lastHilbertValue, lastKey,
                                                    mbrPredicate,
                                                    (low, high) -> suffixRange.overlaps(low, high)),
                                            cursorLimitManager, timer))), executor)
                                    .filter(itemSlot -> lastHilbertValue == null || lastKey == null ||
                                                        itemSlot.compareHilbertValueAndKey(lastHilbertValue, lastKey) > 0)
                                    .filter(itemSlot -> containsPredicate.test(itemSlot.getPosition()))
                                    .filter(itemSlot -> suffixRange.contains(itemSlot.getKeySuffix()))
                                    .map(itemSlot -> {
                                        final List<Object> keyItems = Lists.newArrayList();
                                        if (prefixTuple != null) {
                                            keyItems.addAll(prefixTuple.getItems());
                                        }
                                        keyItems.addAll(coordinateReconstructor.reconstruct(itemSlot.getPosition().getCoordinates()));
    ...
    /**
     * Insert or delete the R-tree items for the given index entries.
     *
     * @param state the maintainer state
     * @param config the R-tree configuration
     * @param prefixSize number of leading (prefix/grouping) columns partitioning the index into separate R-trees
     * @param dimensionsSize number of coordinate columns following the prefix
     * @param savedRecord the record being indexed
     * @param remove {@code true} to delete, {@code false} to insert or update
     * @param indexEntries the index entries for the record
     * @param pointBuilder builds the stored point from an entry's coordinate columns
     * @param <M> the record message type
     * @return a future that completes when all entries have been maintained
     */
    @Nonnull
    public static <M extends Message> CompletableFuture<Void> update(@Nonnull final IndexMaintainerState state,
                                                                     @Nonnull final RTree.Config config,
                                                                     final int prefixSize,
                                                                     ...
                                                                     @Nonnull final PointBuilder pointBuilder) {
        final Subspace indexSubspace = state.indexSubspace;
        final Subspace nodeSlotIndexSubspace = nodeSlotIndexSubspace(state);
        final Executor executor = state.context.getExecutor();
        final FDBStoreTimer timer = Objects.requireNonNull(state.context.getTimer());
        final var futures = indexEntries.stream().map(indexEntry -> {
            ...
            return state.context.doWithWriteLock(new LockIdentifier(rtSubspace), () -> {
                final RTree.Point point =
                        pointBuilder.build(indexKeyItems.subList(prefixSize, prefixSize + dimensionsSize));
    ...
    static class Continuation implements RecordCursorContinuation {
        @Nullable
        ...
        Continuation(@Nullable final BigInteger lastHilbertValue, @Nullable final Tuple lastKey) {
        ...
        @Nullable
        static Continuation fromBytes(@Nullable byte[] continuationBytes) {
