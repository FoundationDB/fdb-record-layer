 ...
 *
 * @param <T> a parent planner value type of all possible sub classes of {@link Value} that this rule could match
 * @see com.apple.foundationdb.record.query.plan.cascades
 * @see PlannerRuleCall
 */
@API(API.Status.EXPERIMENTAL)
public abstract class ValueSimplificationRule<T extends Value> implements PlannerRule<Value, ValueSimplificationRuleCall, T> {
    @Nonnull
    private final BindingMatcher<T> matcher;

    public ValueSimplificationRule(@Nonnull BindingMatcher<T> matcher) {
