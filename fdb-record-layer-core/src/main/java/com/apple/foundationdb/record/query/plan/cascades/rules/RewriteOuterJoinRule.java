 ...
 *
 * <p>If the null-supplying side is already a single {@link SelectExpression}, the {@code ON} predicates are folded
...
public class RewriteOuterJoinRule extends ExplorationCascadesRule<OuterJoinExpression> {
    ...
    public void onMatch(@Nonnull final ExplorationCascadesRuleCall call) {
        ...
        // children are plain `ForEach` quantifiers. (Its constructor enforces this.)
        Verify.verify(!preservedQun.isNullOnEmpty());
        Verify.verify(!nullSupplyingQun.isNullOnEmpty());

        // Build the inner `SelectExpression` carrying the ON predicates.
        final SelectExpression innerSelect = buildInnerSelect(outerJoinExpression, nullSupplyingQun);
    ...
    /**
     * Builds the inner {@link SelectExpression} that the null-on-empty quantifier will range over. If the
     * null-supplying side is already a single {@link SelectExpression}, fold the {@code ON} predicates directly into
    ...
    private static SelectExpression buildInnerSelect(@Nonnull final OuterJoinExpression outerJoinExpression,
        ...
        if (exploratory.size() == 1 && exploratory.iterator().next() instanceof SelectExpression existingSelect) {
            ...
            // `existingSelect.getResultValue()`, which is what `nullSupplyingQun` flows. Once translated, the
            // predicates resolve against the quantifier aliases of `existingSelect` and can be added directly.
            final TranslationMap aliasRewire = TranslationMap.regularBuilder()
                    .when(nullSupplyingQun.getAlias())
                    .then((sourceAlias, leafValue) -> existingSelect.getResultValue())
                    .build();
            final List<QueryPredicate> translatedOnPredicates = outerJoinExpression.getJoinPredicates().stream()
                    .map(p -> p.translateCorrelations(aliasRewire, true))
                    .collect(ImmutableList.toImmutableList());
            return new SelectExpression(
                    existingSelect.getResultValue(),
                    ImmutableList.copyOf(existingSelect.getQuantifiers()),
                    ImmutableList.<QueryPredicate>builder()
                            .addAll(existingSelect.getPredicates())
                            .addAll(translatedOnPredicates)
                            .build());
        }
        return new SelectExpression(
