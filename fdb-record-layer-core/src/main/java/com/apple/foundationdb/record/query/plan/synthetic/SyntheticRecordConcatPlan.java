...
public class SyntheticRecordConcatPlan implements SyntheticRecordFromStoredRecordPlan  {
    ...
    public static RecordCursor<FDBSyntheticRecord> addDistinct(RecordCursor<FDBSyntheticRecord> cursor) {
        final Set<Pair<String, Tuple>> seen = new HashSet<>();
        // Synthetic record primary keys are not unique.
        return cursor.filter(r -> seen.add(Pair.of(r.getRecordType().getName(), r.getPrimaryKey())));
    }
