 ...
 * {'a', 'b'}
 * then the records are expected to be sorted by at least {'a', 'b'} ({'a', 'b', 'c'} is also compatible.
 *
 * @param <T> the type of cursor result the aggregator will be processing
 * @param <M> the type of content (message) that is in the store
 */
public class GroupAggregator<T extends FDBRecord<M>, M extends Message> {
    @Nonnull
    ...
    @Nullable
    private List<Object> currentGroup; // The current group (evaluated). This will be used to decide if the next record is a group break
    @Nullable
    private List<Object> previousGroupResult; // The previous completed group result - with both grouping criteria and accumulated values
    ...
    /**
     * Accept the next record, decide whether this next record constitutes a group break, following these rules:
     * <UL>
     ...
     * </UL>
     *
     * @param nextRecord the next record to process
     *
     * @return true IFF the next record provided constitutes a group break
     */
    public boolean apply(@Nonnull RecordCursorResult<T> nextRecord) {
        if (!nextRecord.hasNext()) {
            finalizeGroup(null);
            return true;
        } else {
            List<Object> nextGroup = eval(nextRecord, groupCriteria);
            ...
            accumulate(nextRecord);
            return groupBreak;
        }
    }

    /**
     * Get the last completed group result. This will return the group that was completed last (prior to the last group break).
     *
    ...
    private void accumulate(final @Nonnull RecordCursorResult<T> nextRecord) {
        EvaluationContext nestedContext = context.withBinding(alias, nextRecord.get().getRecord());
        accumulator.accumulate(store, nestedContext, nextRecord.get(), nextRecord.get().getRecord());
    }

    private List<Object> eval(final RecordCursorResult<T> record, List<Value> values) {
        final EvaluationContext nestedContext = context.withBinding(alias, record.get().getRecord());
        return values.stream()
                .map(value -> value.eval(store, nestedContext, record.get(), record.get().getRecord()))
