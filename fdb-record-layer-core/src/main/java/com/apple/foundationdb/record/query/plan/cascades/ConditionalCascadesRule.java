...
public class ConditionalCascadesRule<T extends RelationalExpression, R extends CascadesRule<T>> extends AbstractCascadesRule<T> {
     ...
     */
    private final boolean onlyOnPrunedInputs;
    ...
    public ConditionalCascadesRule(@Nonnull final List<R> rules) {
        ...
        this.onlyOnPrunedInputs = deriveOnlyOnPrunedInputs(rules);
    }
    ...
    @Override
    public boolean onlyOnPrunedInputs() {
        return onlyOnPrunedInputs;
    }
    ...
    /**
     * Derives the {@code onlyOnPrunedInputs} flag for a group of inner rules. All rules must agree: either every
     * rule is a {@link OnPrunedInputsRule} (and the conditional rule inherits that scheduling behaviour) or none of them
     * is. A mix of on-pruned-input and normal rules is rejected at construction time, because the two kinds are scheduled
    ...
    private static <T, R extends CascadesRule<T>> boolean deriveOnlyOnPrunedInputs(@Nonnull final List<R> rules) {
        ...
        for (final R rule : rules) {
            Verify.verify(rule.onlyOnPrunedInputs() == first,
                    "all inner rules of a `ConditionalCascadesRule` must agree on `onlyOnPrunedInputs`");
        }
        return first;
    }
