...
public class ExplainPlanProperty implements ExpressionProperty<ExplainTokens> {
    private static final ExplainPlanProperty EXPLAIN_PLAN = new ExplainPlanProperty();

    private ExplainPlanProperty() {
        // prevent outside instantiation
    }

    @Nonnull
    @Override
    public RelationalExpressionVisitor<ExplainTokens> createVisitor() {
        return ExpressionProperty.toExpressionVisitor(new ExplainPlanVisitor(Integer.MAX_VALUE));
    }

    @Nonnull
    public ExplainTokens evaluate(@Nonnull final Reference reference) {
        return evaluate(reference.getAsPlan());
    }

    @Nonnull
    public ExplainTokens evaluate(@Nonnull final RecordQueryPlan plan) {
        return createVisitor().visit(plan);
    }

    @Nonnull
    public static ExplainPlanProperty explainPlan() {
        return EXPLAIN_PLAN;
    }
    ...
    public static class ExplainPlanVisitor extends ExplainTokens implements RecordQueryPlanVisitor<ExplainTokens> {
