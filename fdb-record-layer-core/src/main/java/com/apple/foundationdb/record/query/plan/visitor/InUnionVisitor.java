...
public class InUnionVisitor extends RecordQueryPlannerSubstitutionVisitor {
    ...
    public RecordQueryPlan postVisit(@Nonnull RecordQueryPlan recordQueryPlan) {
        if (recordQueryPlan instanceof RecordQueryInUnionOnKeyExpressionPlan) {
            ...
            @Nullable RecordQueryPlan removedFetchPlan = removeIndexFetch(inUnionPlan.getChild(), inUnionPlan.getRequiredFields());
            if (removedFetchPlan == null) {
                return recordQueryPlan;
            }

            recordQueryPlan = new RecordQueryFetchFromPartialRecordPlan(
                    inUnionPlan.withChild(GroupExpressionRef.of(removedFetchPlan)),
