...
import com.apple.foundationdb.record.query.plan.cascades.values.EuclideanDistanceRankValue;
...
public class VectorIndexExpansionVisitor extends KeyExpressionExpansionVisitor implements ExpansionVisitor<KeyExpressionExpansionVisitor.VisitorState> {
    ...
    public MatchCandidate expand(@Nonnull final Supplier<Quantifier.ForEach> baseQuantifierSupplier,
        ...
        final var distanceRankValuePlaceHolder = new EuclideanDistanceRankValue(ImmutableList.copyOf(keyValues), ImmutableList.copyOf(valueValues))
                .asPlaceholder(newParameterAlias());

        allExpansionsBuilder.add(keyValueExpansion);
        allExpansionsBuilder.add(GraphExpansion.ofPlaceholder(distanceRankValuePlaceHolder));
