...
public class VectorIndexScanMatchCandidate implements ScanWithFetchMatchCandidate, ValueIndexLikeMatchCandidate {
    ...
    @Nonnull
    private final Placeholder distancePredicatePlaceholder;
    ...
    public VectorIndexScanMatchCandidate(@Nonnull final Index index,
                                         ...
                                         @Nonnull final Placeholder distancePredicatePlaceholder,
        ...
        this.distancePredicatePlaceholder = distancePredicatePlaceholder;
    ...
    public RecordQueryPlan toEquivalentPlan(@Nonnull final PartialMatch partialMatch,
        ...
        final var comparisons = extractRankComparison(comparisonRanges);
        ...
        return tryFetchCoveringIndexScan(partialMatch, planContext, memoizer, comparisonRanges, reverseScanOrder, baseRecordType)
                .orElseGet(() ->
                        new RecordQueryIndexPlan(index.getName(),
                                primaryKey,
                                VectorIndexScanComparisons.byValue(toScanComparisons(comparisons.getLeft()), comparisons.getRight(), null),
    ...
    private static NonnullPair<List<ComparisonRange>, Comparisons.DistanceRankValueComparison> extractRankComparison(@Nonnull final List<ComparisonRange> comparisonRanges) {
        final var nonRankComparisonsBuilder = ImmutableList.<ComparisonRange>builder();
        final var rankComparisonsBuilder = ImmutableList.<Comparisons.DistanceRankValueComparison>builder();
        for (final var comparisonRange : comparisonRanges) {
            if (comparisonRange.isEquality() && comparisonRange.getEqualityComparison() instanceof Comparisons.DistanceRankValueComparison) {
                rankComparisonsBuilder.add((Comparisons.DistanceRankValueComparison)comparisonRange.getEqualityComparison());
            } else if (comparisonRange.isInequality() && comparisonRange.getInequalityComparisons().stream().allMatch(comp -> comp instanceof Comparisons.DistanceRankValueComparison)) {
                rankComparisonsBuilder.addAll(comparisonRange.getInequalityComparisons().stream().map(comp -> (Comparisons.DistanceRankValueComparison)comp).collect(ImmutableList.toImmutableList()));
            } else {
                nonRankComparisonsBuilder.add(comparisonRange);
            }
        }
        final var rankComparisons = rankComparisonsBuilder.build();
        Verify.verify(rankComparisons.size() == 1);
        return NonnullPair.of(nonRankComparisonsBuilder.build(), rankComparisons.get(0));
    }
    ...
    @Nonnull
    public Placeholder getDistancePredicatePlaceholder() {
        return distancePredicatePlaceholder;
    }
}
