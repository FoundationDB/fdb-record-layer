 ...
 * SUM, MIN, AVG etc.).
 *
 * @param <S> the state type. The state holds the values as they are being aggregated, and where the eventualresult is derived from.
 */
public interface AggregateValue<S> extends Value.CompileTimeValue {
     ...
     */
    @Nonnull
    List<Object> finish(S currentState);
}
