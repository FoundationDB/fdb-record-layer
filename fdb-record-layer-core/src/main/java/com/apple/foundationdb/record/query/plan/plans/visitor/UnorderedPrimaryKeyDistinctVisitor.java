 ...
 *
 */
public class UnorderedPrimaryKeyDistinctVisitor extends BaseMetadataVisitor {

    public UnorderedPrimaryKeyDistinctVisitor(RecordMetaData recordMetaData) {
    ...
    public Visitable visit(final Visitable recordQueryPlan, final Visitable parentRecordQueryPlan) {
        if (recordQueryPlan instanceof RecordQueryUnorderedPrimaryKeyDistinctPlan) {
            ...
            if (isLookupMoveable(distinctPlan.getChild())) {
                distinctPlan.getChild().setIsIndexFetch(false);
                distinctPlan.setIsIndexFetch(true);
            }
        }
