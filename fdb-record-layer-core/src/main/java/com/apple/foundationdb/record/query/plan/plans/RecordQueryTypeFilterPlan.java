...
public class RecordQueryTypeFilterPlan implements RecordQueryPlanWithChild, TypeFilterExpression {
    ...
    public RecordQueryTypeFilterPlan(@Nonnull Quantifier.Physical inner, @Nonnull Collection<String> recordTypes, @Nullable Type.Record resultType) {
        this.inner = inner;
    ...
    public String explain(@Nonnull final Formatter formatter) {
        return "typeFilter(" + getInnerPlan().explain(formatter) + ", " + recordTypes.stream().map(recordType -> "'" + recordType + "'").collect(Collectors.joining(", ")) + ")";
    }
