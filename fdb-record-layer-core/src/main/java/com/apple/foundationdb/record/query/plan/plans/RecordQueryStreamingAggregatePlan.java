 ...
 * This plan will:
 * <UL>
 * <LI>Group items by its list of group {@link Value}s. This list (order matters) holds the {@link Value}s that will decide
...
public class RecordQueryStreamingAggregatePlan implements RecordQueryPlanWithChild {
    ...
    public <M extends Message> RecordCursor<FDBQueriedRecord<M>> execute(@Nonnull FDBRecordStoreBase<M> store,
        ...
        final RecordCursor<FDBQueriedRecord<M>> innerCursor = getInnerPlan().execute(store, context, continuation, executeProperties.clearSkipAndLimit());
        GroupAggregator<FDBQueriedRecord<M>, M> groupAggregator = new GroupAggregator(groupByCriteria, aggregateValues, store, context, inner.getAlias());
    ...
    public String toString() {
        return getInnerPlan().toString() + " | GROUP BY " + groupByCriteria + ", AGGREGATE BY " + aggregateValues;
    }
