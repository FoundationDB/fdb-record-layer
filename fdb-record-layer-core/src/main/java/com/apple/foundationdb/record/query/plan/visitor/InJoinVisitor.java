...
public class InJoinVisitor extends RecordQueryPlannerSubstitutionVisitor {
    public InJoinVisitor(final RecordMetaData recordMetadata, final PlannableIndexTypes indexTypes, @Nullable final KeyExpression commonPrimaryKey) {
    ...
    public RecordQueryPlan postVisit(RecordQueryPlan recordQueryPlan) {
        if (recordQueryPlan instanceof RecordQueryInJoinPlan) {
            ...
            @Nullable FetchIndexRecords fetchIndexRecords = resolveFetchIndexRecordsFromPlan(inJoinPlan.getChild());
