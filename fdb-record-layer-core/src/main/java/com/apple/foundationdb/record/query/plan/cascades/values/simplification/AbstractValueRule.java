 ...
 *
 * @param <R> the type of the result being yielded by rule implementations
 * @param <C> the type of rule call that is used in calls to {@link #onMatch(PlannerRuleCall)} )}
 * @param <T> a value type of all possible subclasses of {@link Value} that this rule could match
 * @see com.apple.foundationdb.record.query.plan.cascades
 * @see PlannerRuleCall
 */
@API(API.Status.EXPERIMENTAL)
public abstract class AbstractValueRule<R, C extends AbstractValueRuleCall<R, C>, T extends Value> implements PlannerRule<R, C, T> {
    @Nonnull
    private final BindingMatcher<T> matcher;

    public AbstractValueRule(@Nonnull BindingMatcher<T> matcher) {
