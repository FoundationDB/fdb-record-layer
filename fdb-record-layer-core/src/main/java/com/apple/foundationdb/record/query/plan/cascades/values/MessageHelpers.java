...
public class MessageHelpers {
    ...
    public static Object coerceObject(@Nullable final CoercionTrieNode coercionsTrie,
        ...
        if (coercionsTrie == null) {
            ...
            // { repeated <type> values = 1; }. Otherwise, when the evaluated value is a plain List (e.g., from an array
            // literal or from `unwrapIfArray()`), subsequently setting it on the protobuf builder causes `setField()`
            // to reject the `List<>` with a type mismatch error.
            if (targetType.isNullable() && targetType.isArray() && current instanceof List) {
                Verify.verify(targetDescriptor instanceof Descriptors.Descriptor);
