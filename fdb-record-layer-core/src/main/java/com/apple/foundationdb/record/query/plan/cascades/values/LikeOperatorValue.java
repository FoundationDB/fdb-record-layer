...
public class LikeOperatorValue extends AbstractValue implements BooleanValue {
     ...
     */
    public LikeOperatorValue(@Nonnull Value srcChild, @Nonnull Value patternChild, @Nonnull Value escapeChild) {
    ...
    public <M extends Message> Object eval(@Nonnull final FDBRecordStoreBase<M> store, @Nonnull final EvaluationContext context) {
        return like((String)srcChild.eval(store, context), (String)patternChild.eval(store, context), (String)escapeChild.eval(store, context));
    }

    @Nonnull
    @Override
    public String explain(@Nonnull final Formatter formatter) {
        return srcChild.explain(formatter) + " LIKE " + patternChild.explain(formatter) + " ESCAPE " + escapeChild.explain(formatter);
    }
    ...
    public Optional<QueryPredicate> toQueryPredicate(@Nullable final TypeRepository typeRepository, @Nonnull final CorrelationIdentifier innermostAlias) {
        return Optional.of(new ValuePredicate(srcChild, new Comparisons.SimpleComparison(Comparisons.Type.LIKE, patternChild)));
    }
    ...
    private static Value encapsulate(@Nonnull BuiltInFunction<Value> builtInFunction,
        ...
        Type escapeType = arguments.get(2).getResultType();
        ...
        return new LikeOperatorValue((Value) arguments.get(0), (Value) arguments.get(1), (Value) arguments.get(2));
    }
    ...
    public static class LikeFn extends BuiltInFunction<Value> {
        public LikeFn () {
            super("like",
                    ImmutableList.of(Type.primitiveType(TypeCode.STRING), Type.primitiveType(TypeCode.STRING), Type.primitiveType(TypeCode.STRING)), LikeOperatorValue::encapsulate);
        }
    }

    private static final String[] SEARCH = new String[]{"%", "_", "|", ".", "^", "$", "\\", "*", "+", "?", "[", "]", "{", "}", "(", ")"};
    private static final String[] REPLACE = new String[]{".*", ".", "\\|", "\\.", "\\^", "\\$", "\\\\", "\\*", "\\+", "\\?", "\\[", "\\]", "\\{", "\\}", "\\(", "\\)"};

    public static Boolean like(@Nullable String lhs, @Nullable String rhs, @Nullable String escapeChar) {
        ...
        Pattern pattern = Pattern.compile("^" + StringUtils.replaceEach(rhs, search, replace) + "$");
