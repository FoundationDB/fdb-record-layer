...
record ClusterMetadataDelta(@Nonnull StatsOp statsOp,
     ...
     * <p>
     * The high-water mark {@link ClusterMetadata#maxEverNumPrimaryVectors()} is raised here to the primary count this
     * delta produces, since {@link ClusterMetadata} rejects a mark below its own count rather than correcting it.
     * Because the deltas replay in order, each one raising the mark as it goes, the folded result records the peak the
     * cluster passed through between compactions and not merely its count at either end.
     *
    ...
    ClusterMetadata applyTo(@Nonnull final ClusterMetadata clusterMetadata) {
        ...
        return new ClusterMetadata(clusterMetadata.id(), newNumPrimaryUnderreplicatedVectors,
                ...
                Math.max(clusterMetadata.maxEverNumPrimaryVectors(), numPrimaryVectors));
    }
